...
usage() {
  cat <<'EOF'
Builds client/server locally, zips artifacts, and deploys them to EC2 by streaming ZIPs
into unzip (FIFO) so the EC2 disk doesn't need to hold the ZIPs.
...
Example:
  ...
  REMOTE_APP_DIR=/home/ec2-user/61c \
  bash deploy/scripts/deploy-artifacts-stream.sh
EOF
}
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/61c-deploy-stream.XXXXXX")"
cleanup() {
  rm -rf "$WORK_DIR"
}
trap cleanup EXIT
...
echo "Preparing EC2 (stop services, pre-clean releases) ..."
ssh "${SSH_OPTS[@]}" "$EC2_USER@$EC2_HOST" \
  "REMOTE_APP_DIR='$REMOTE_APP_DIR' REMOTE_STAGING_DIR='$REMOTE_STAGING_DIR' REMOTE_KEEP_RELEASES='$REMOTE_KEEP_RELEASES' REMOTE_CLIENT_RELEASE_DIR='$REMOTE_CLIENT_RELEASE_DIR' REMOTE_SERVER_RELEASE_DIR='$REMOTE_SERVER_RELEASE_DIR' bash -s" <<'REMOTE'
set -euo pipefail
...
# Stop running services before deleting old releases (prevents deleting files in-use).
pm2 delete 61c-client >/dev/null 2>&1 || true
pm2 delete 61c-server >/dev/null 2>&1 || true

# Pre-clean releases to fit into small disks.
...
echo "Streaming client ZIP to EC2 and extracting ..."
cat "$WORK_DIR/client-build.zip" | ssh "${SSH_OPTS[@]}" "$EC2_USER@$EC2_HOST" \
  "REMOTE_STAGING_DIR='$REMOTE_STAGING_DIR' REMOTE_CLIENT_RELEASE_DIR='$REMOTE_CLIENT_RELEASE_DIR' bash -s" <<'REMOTE'
set -euo pipefail
export PATH="$HOME/.local/node/bin:$PATH"

FIFO="$REMOTE_STAGING_DIR/client-build.zip.fifo"
rm -f "$FIFO"
mkfifo "$FIFO"

unzip -q -o -d "$REMOTE_CLIENT_RELEASE_DIR" "$FIFO" &
UNZIP_PID=$!

# Consume the ZIP bytes from SSH stdin and write them into the FIFO.
cat > "$FIFO"
wait "$UNZIP_PID"
...
echo "Streaming server ZIP to EC2 and extracting ..."
cat "$WORK_DIR/server-build.zip" | ssh "${SSH_OPTS[@]}" "$EC2_USER@$EC2_HOST" \
  "REMOTE_STAGING_DIR='$REMOTE_STAGING_DIR' REMOTE_SERVER_RELEASE_DIR='$REMOTE_SERVER_RELEASE_DIR' bash -s" <<'REMOTE'
set -euo pipefail
export PATH="$HOME/.local/node/bin:$PATH"

FIFO="$REMOTE_STAGING_DIR/server-build.zip.fifo"
rm -f "$FIFO"
mkfifo "$FIFO"

unzip -q -o -d "$REMOTE_SERVER_RELEASE_DIR" "$FIFO" &
UNZIP_PID=$!
